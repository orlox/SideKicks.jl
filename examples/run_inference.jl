...
system_id = "vfts243" 
system_dir = String(@__DIR__)*"/$system_id/"

## 

# Obtain observations and priors from external files
obs_string = open(system_dir * "observations.jl", "r") do file
        read(file, String)
end
obs = eval(Meta.parse(obs_string))


priors_string = open(system_dir * "priors.jl", "r") do file
    read(file, String)
end
...
SideKicks.SaveResults(system_dir *"results-medium.hdf5", mcmcStruct, obs_string, priors_string)
